...
로그인 없이는 결과 화면을 볼 수 없는 구조라, 아래 계정으로 로그인하면 대기 시간 없이 미리 생성된 3일 식단을 바로 확인할 수 있습니다.

- **접속 주소**: (배포된 서비스 URL)/auth/login
- **이메일**: `judge@bobplan.demo`
- **비밀번호**: `BobPlan2026!`

로그인 후 마이페이지 또는 최근 생성한 식단 링크로 들어가면 샘플 식단을 볼 수 있고, "식단 만들기"로 직접 새 식단을 생성해 실시간 AI 생성 과정도 체험할 수 있습니다.

데모 계정과 샘플 식단은 `node prisma/seed-demo.js` 스크립트로 생성됩니다 (최초 1회 실행, `.env`에 `GEMINI_API_KEY`와 `DATABASE_URL`이 설정되어 있어야 합니다).
